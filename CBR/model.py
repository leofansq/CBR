...
class Decoder(nn.Module):
    ...
    def __init__(self, num_ch_enc, num_class=2, type=''):
        ...
        for i in range(3, -1, -1):
            if type == 'transform_decoder':
                num_ch_in = 1024 if i == 3 else self.num_ch_dec[i + 1]
            else:
                num_ch_in = 1024 if i == 3 else self.num_ch_dec[i + 1]
        ...
        self.convs["topview"] = Conv3x3(16, self.num_output_channels)
    ...
    def forward(self, x, is_training=True):
        ...
        if is_training:
            seg = self.convs["topview"](x[:,:16,:,:])
        else:
            softmax = nn.Softmax2d()
            seg = softmax(self.convs["topview"](x[:,:16,:,:]))
