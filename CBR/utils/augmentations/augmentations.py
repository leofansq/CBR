...
from ..kitti_utils import convertRot2Alpha, convertAlpha2Rot, refresh_attributes
...
#     def __call__(self, img, bev_seg, fv_seg, objs, calib):
#         if isinstance(img, np.ndarray):
#             img = Image.fromarray(img, mode="RGB")
#             self.PIL2Numpy = True

#         for a in self.augmentations:
#             img, bev_seg, fv_seg, objs, calib = a(img, bev_seg, fv_seg, objs, calib)

#         if self.PIL2Numpy:
#             img = np.array(img)

#         return img, bev_seg, fv_seg, objs, calib
...
#             bev_seg = bev_seg.transpose(Image.FLIP_LEFT_RIGHT)
#             fv_seg = fv_seg.transpose(Image.FLIP_LEFT_RIGHT)
...
class Compose(object):
    ...
    def __call__(self, img, bev_seg, fv_seg, objs):
        if isinstance(img, np.ndarray):
            img = Image.fromarray(img, mode="RGB")
            self.PIL2Numpy = True

        for a in self.augmentations:
            img, bev_seg, fv_seg, objs = a(img, bev_seg, fv_seg, objs)

        if self.PIL2Numpy:
            img = np.array(img)

        return img, bev_seg, fv_seg, objs
